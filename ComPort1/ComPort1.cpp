...
VOID WINAPI ThreadSend(PVOID* dummy) {
	while (true) {
		...
		message = "Получено сообщение от COM 1: " + message;
...
int main(void) {
	...
	cout << "COM 1 порт успешно сконфигурирован." << endl;
	GetCommState(hCom, &dcb);
	...
	cout << "Принимайте и отправляйте сообщения в COM 2 (через Enter).\n" << endl;
	char sReceivedChar;
